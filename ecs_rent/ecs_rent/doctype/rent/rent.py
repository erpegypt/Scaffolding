	...
	def validate(doc):
		if doc.rent_type == "Daily":
			for x in doc.time_logs:
				x.rate = frappe.db.get_value('Item Price', {"item_code": x.item_code, "selling" : 1,"uom": "Daily"}, 'price_list_rate') or 0
				x.income_account = frappe.db.get_single_value('Company', 'default_income_account')
		else:
			for x in doc.time_logs:
				x.rate = frappe.db.get_value('Item Price', {"item_code": x.item_code, "selling" : 1,"uom": "Monthly"}, 'price_list_rate') or 0
				x.income_account = frappe.db.get_single_value('Company', 'default_income_account')
